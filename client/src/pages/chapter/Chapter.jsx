...
const Chapter = () => {
  ...
  const chapter = useCourseStore((state) =>
    state.course?.chapters.find((c) => c._id === chapterId)
  );
  ...
  const generateQuiz = async () => {
    ...
    try {
      ...
      const quizResp = await Axios.post(
        APP_SERVER + "/api/quiz/generate",
        { courseId, chapterId, topic },
        {
          headers: {
            Authorization: "Bearer " + Cookies.get("token"),
          },
  ...
  const handleChaterCompletion = async (chapterId) => {
    try {
      const completionResp = await Axios.post(
        APP_SERVER + `/api/course/chapter/complete`,
        { courseId: course._id, chapterId },
        {
          headers: {
            Authorization: "Bearer " + Cookies.get("token"),
          },
      ...
      <div className="w-full max-w-screen-xl h-full">
        ...
        <Breadcrumbs className="px-0 bg-color-white my-2">
          <Typography
            ...
            variant="p"
            onClick={() => navigate(`/app/course/${course._id}`)}
          >
            {course?.title}
          </Typography>
        ...
        <div className="w-full py-4 flex gap-2 justify-between">
          {findPrevChapterId() ? (
            <Button
              ...
              onClick={() =>
                navigate(
                  `/app/course/${course._id}/chapter/${findPrevChapterId()}`
                )
              }
            >
              Back
          ...
          <div>
            <Button
              variant={chapter?.completed ? "filled" : "outlined"}
              onClick={() => handleChaterCompletion(chapter._id)}
            ...
            {findNextChapterId() ? (
              <Button
                ...
                onClick={() =>
                  navigate(
                    `/app/course/${course._id}/chapter/${findNextChapterId()}`
                  )
                }
              >
                Next chapter
